...
func _ready():
	...
	for level_path in GameData.level_paths:
		var level_button = LevelSelectorItem.instantiate()
		LevelGridContainer.add_child(level_button)
		level_button.connect("pressed", Callable(self, "_on_LevelButton_pressed").bind(counter))
...
func _on_LevelButton_pressed(id):
	...
	get_tree().change_scene_to_file("res://engine/gameplay_handler.tscn")
