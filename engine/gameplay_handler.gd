...
func _ready():
	...
	for block in instanced_level.get_node("static").get_children():
		...
		#block.mode = RigidBody2D.FREEZE_MODE_STATIC
		block.freeze = true
		
		# Enable mouse over
		block.input_pickable = true
		block.connect("mouse_entered", Callable(self, "_on_StaticBlock_mouse_entered"))
		block.connect("mouse_exited", Callable(self, "_on_StaticBlock_mouse_exited"))
	...
	if current_game_state == game_states.active:		
		if Input.is_action_just_pressed("select") and can_place_block:
			...
			if new_block != null:
				...
				new_block.connect("mouse_entered", Callable(self, "_on_StaticBlock_mouse_entered"))
				new_block.connect("mouse_exited", Callable(self, "_on_StaticBlock_mouse_exited"))
...
func _update_block_preview():
	if active_block_list.size() > 0:
		block_preview.texture = active_block_list[0].get_node("Sprite2D").texture
